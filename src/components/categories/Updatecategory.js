...
class Updatecategory extends Component {
  ...
  handleSubmit = (e) => {
    ...
    const { category_name, company_id } = this.state;
    console.log(JSON.stringify(this.state));
    axios
      .put(
        url +
          "/api/v2/categories/" +
          JSON.parse(localStorage.getItem("categorytoupdate")).id,
        {
          category: {
            category_name,
            company_id,
                ...
                <Form>
                  <Row>
                    <Col className="pl-1" md="6">
                      <Form.Group>
                        <label htmlFor="category name">La categorie</label>
                        <Form.Control
                          name="category_name"
                          placeholder={
                            JSON.parse(localStorage.getItem("categorytoupdate"))
                              .category_name
                          }
